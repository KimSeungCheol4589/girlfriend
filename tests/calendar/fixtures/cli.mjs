#!/usr/bin/env node
/**
 * CAL-001 E2E 픽스처 준비·정리 — 로컬 Supabase 전용.
 *
...
export const CALENDAR_PREFIX = 'calendar-e2e';
export const CALENDAR_ACCOUNT_KEYS = /** @type {const} */ (['a', 'b', 'c']);

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = resolve(HERE, '..', '..', '..');
const ACCOUNTS_PATH = join(REPO_ROOT, '.agent-runtime', 'calendar-e2e', 'accounts.json');

function calendarEmail(key) {
  const email = syntheticEmail(CALENDAR_PREFIX, key);
  // 방어: 접두사가 바뀌어 다른 스위트 계정을 건드리는 일이 없게 한다.
  if (!email.startsWith(`${CALENDAR_PREFIX}-`) || !isSyntheticEmail(email)) {
    fail('CAL-001 합성 계정만 다룹니다.');
  }
  return email;
}
...
function removeBootstrap(config) {
  const problems = [];
  const creators = [calendarEmail('a'), calendarEmail('c')];
...
async function setup(config) {
  ...
  const emails = CALENDAR_ACCOUNT_KEYS.map(calendarEmail);
  ...
  const accounts = {};
  for (const key of CALENDAR_ACCOUNT_KEYS) {
    const email = calendarEmail(key);
