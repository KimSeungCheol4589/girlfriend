#!/usr/bin/env node
/**
 * FOOD-001 E2E 픽스처 준비·정리 — 로컬 Supabase 전용.
 *
 *   node tests/restaurants/fixtures/cli.mjs setup
 *   node tests/restaurants/fixtures/cli.mjs teardown
 *   node tests/restaurants/fixtures/cli.mjs status
 *
 * 네임스페이스: 합성 계정 `food001-{a,b,c}@test.invalid`만 만들고 지운다.
 *   - AUTH(`auth-e2e-*`)·MEM 픽스처 계정은 읽지도 지우지도 않는다. 접두사는 코드에 고정돼 있고
 *     `AUTH_TEST_EMAIL_PREFIX` 환경 변수를 따르지 않는다.
 *   - 인증 픽스처의 안전장치(loopback 주소만, `.invalid` 도메인만, production 금지)를 그대로 쓴다
 *     (`tests/auth/fixtures/*.mjs`를 읽기 전용으로 가져온다).
 *   - 비밀번호는 실행마다 새로 만들고 `.agent-runtime/food001-e2e/accounts.json`(Git 제외, 0600)에만 둔다.
 *   - 키·비밀번호·초대 토큰은 어떤 출력에도 넣지 않는다.
 *   - 컨테이너를 만들거나 지우거나 재시작하지 않는다. 이미 떠 있는 DB 컨테이너에서 psql만 쓴다.
 *
 * setup이 만드는 상태
 *   A(food001-a): 공간 생성 → B 초대 → B(food001-b) 수락 → 두 사람 공간
 *   C(food001-c): 별도 공간(외부 계정 역할)
 *   setup은 먼저 food001 계정의 공간·요청 기록을 지워 매 실행을 같은 출발점에서 시작한다.
 */
...
import { FixtureError, fail, generatePassword, isSyntheticEmail, readFixtureEnv, syntheticEmail } from '../../auth/fixtures/env.mjs';
import {
  canRunSql,
...
export const FOOD_PREFIX = 'food001';
export const FOOD_ACCOUNT_KEYS = /** @type {const} */ (['a', 'b', 'c']);

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = resolve(HERE, '..', '..', '..');
const ACCOUNTS_PATH = join(REPO_ROOT, '.agent-runtime', 'food001-e2e', 'accounts.json');

function foodEmail(key) {
  const email = syntheticEmail(FOOD_PREFIX, key);
  // 방어: 접두사가 바뀌어 다른 스위트 계정을 건드리는 일이 없게 한다.
  if (!email.startsWith(`${FOOD_PREFIX}-`) || !isSyntheticEmail(email)) {
    fail('FOOD-001 합성 계정만 다룹니다.');
  }
  return email;
}

function saveAccounts(accounts) {
  if (!ACCOUNTS_PATH.includes('.agent-runtime')) fail('계정 정보는 .agent-runtime 아래에만 저장합니다.');
...
function requireSql(config) {
  if (!canRunSql(config)) {
    fail(
      'AUTH_TEST_DB_CONTAINER가 필요합니다. 공간 생성 허용 목록 등록과 food001 데이터 초기화를 로컬 DB 컨테이너의 psql로 합니다.',
    );
  }
}
...
/**
 * 허용 목록에 food001 계정이 남아 있지 않은지 DB에서 다시 확인한다.
  ...
  return runSql(
    ...
    `do $$ begin
       if exists (select 1 from app_private.bootstrap_creators where email in (${list})) then
         raise exception 'food001 계정이 공간 생성 허용 목록에 남아 있다';
       end if;
     end $$;`,
  );
}

/** food001 계정의 공간·요청 기록이 남아 있지 않은지 확인한다. */
  ...
  return runSql(
    ...
    `do $$ begin
       if exists (select 1 from public.space_members m join auth.users u on u.id = m.user_id
         ...
         raise exception 'food001 공간·요청 기록이 남아 있다';
       end if;
     end $$;`,
  );
}
...
function removeBootstrap(config) {
  const problems = [];
  const creators = [foodEmail('a'), foodEmail('c')];
...
async function setup(config) {
  ...
  const emails = FOOD_ACCOUNT_KEYS.map(foodEmail);

  // 매 실행을 같은 출발점에서: food001 계정의 공간(맛집·후기 포함)과 요청 기록만 지운다.
  checkSql(purgeSyntheticData(config, emails), 'food001 기존 데이터 정리');
  checkSql(verifyDataPurged(config, emails), 'food001 기존 데이터 정리 확인');

  const accounts = {};
  for (const key of FOOD_ACCOUNT_KEYS) {
    const email = foodEmail(key);
...
async function teardown(config) {
  ...
  else console.log('food001 공간·요청 기록 정리 확인');
