#!/usr/bin/env node
/**
 * THEME-001 실제 백엔드 테스트 픽스처 — 로컬 전용.
 *
 *   node tests/customize/fixtures/cli.mjs setup      합성 계정 A·B(같은 공간), C(별도 공간) 준비
 *   node tests/customize/fixtures/cli.mjs teardown   이 픽스처가 만든 공간·파일·요청 기록·계정만 정리
 *   node tests/customize/fixtures/cli.mjs status
 *
 * MEM-001 픽스처(tests/memories/fixtures/cli.mjs)를 복사해 **이름 공간만** 바꾼 것이다.
 * 계정 접두사·계정 파일 경로·환경 변수 이름이 모두 다르므로 두 작업의 데이터가 섞이지 않는다.
 * MEM 픽스처는 이 작업에서 실행하지 않는다(그쪽 계정 `mem-e2e-*`를 건드리지 않기 위해).
 *
 * 안전장치
 *   - Supabase 주소가 loopback이 아니면 멈춘다(인증 픽스처의 assertLocalUrl을 그대로 쓴다).
 *   - 계정은 `theme-e2e-{a,b,c}@test.invalid`만 만들고 지운다. 다른 작업의 계정은 건드리지 않는다.
 *   - 비밀번호는 매번 새로 만들고 `.agent-runtime/customize-e2e/accounts.json`(Git 제외, 0600)에만 저장한다.
 *   - 키·비밀번호·토큰·파일 경로를 출력하지 않는다.
 *   - 컨테이너를 만들거나 지우거나 재시작하지 않는다. 떠 있는 DB 컨테이너에서 psql만 쓴다.
 *   - 만료 정리·트리거를 끄거나 전역 초기화를 하지 않는다. 이 픽스처의 공간만 지운다.
 */
...
const PREFIX = 'theme-e2e';
const KEYS = /** @type {const} */ (['a', 'b', 'c']);
const BUCKET = 'space-assets';
const ACCOUNTS_PATH = join(resolve(process.cwd()), '.agent-runtime', 'customize-e2e', 'accounts.json');
...
function readConfig(env = process.env) {
  ...
  const rawUrl = (env.THEME_TEST_SUPABASE_URL ?? '').trim();
  if (rawUrl === '') fail('THEME_TEST_SUPABASE_URL이 필요합니다.');
  const url = assertLocalUrl(rawUrl, 'Supabase URL');
  const serviceRoleKey = (env.THEME_TEST_SERVICE_ROLE_KEY ?? '').trim();
  if (serviceRoleKey === '') fail('THEME_TEST_SERVICE_ROLE_KEY가 필요합니다(값은 출력하지 않습니다).');
  const anonKey = (env.THEME_TEST_ANON_KEY ?? '').trim();
  if (anonKey === '') fail('THEME_TEST_ANON_KEY가 필요합니다.');
  const dbContainer = (env.THEME_TEST_DB_CONTAINER ?? '').trim();
...
async function setup(config) {
  if (config.dbContainer === '') {
    fail('THEME_TEST_DB_CONTAINER가 없어 공간 생성 허용 목록을 등록할 수 없습니다.');
  }
  ...
  console.log('계정 정보 저장: .agent-runtime/customize-e2e/accounts.json (비밀번호는 출력하지 않습니다)');
  ...
  try {
    ...
    if (!(await currentSpaceId(config, tokenA, accounts.a.userId))) {
      await rpcOrFail(config, tokenA, 'create_space', {
        p_name: 'theme-e2e 공간',
        p_introduction: '꾸미기 검증용 합성 공간',
        p_relationship_start_date: null,
        p_request_id: randomUUID(),
      });
      console.log('A 공간 생성');
    }
    await setNickname(config, tokenA, accounts.a.userId, 'theme-a');
    ...
    await setNickname(config, tokenC, accounts.c.userId, 'theme-c');
  } finally {
    runSql(
      config,
...
async function teardown(config) {
  if (config.dbContainer === '') {
    fail('THEME_TEST_DB_CONTAINER가 없어 합성 공간을 정리할 수 없습니다. 계정도 지우지 않았습니다.');
  }
  ...
  const purge = runSql(
    config,
    `begin;
     create temporary table _theme_spaces on commit drop as ${spacesSql};
     delete from public.memory_photos p where p.space_id in (select id from _theme_spaces);
     delete from public.memories m where m.space_id in (select id from _theme_spaces);
     -- 버전 트리거(새 버전 = 이전 + 1)를 지킨다. 커버가 있는 행만 바꾼다.
     update public.space_settings s
        set cover_asset_id = null,
            version = s.version + 1
      where s.space_id in (select id from _theme_spaces)
        and s.cover_asset_id is not null;
     delete from public.assets a where a.space_id in (select id from _theme_spaces);
     delete from public.spaces s where s.id in (select id from _theme_spaces);
