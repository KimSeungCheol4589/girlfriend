...
/**
 * WISH-001 실제 인증 E2E 설정(로컬 Supabase 전용).
 *
 * - 루트 공통 설정(playwright.config.ts, playwright.auth.config.ts)은 바꾸지 않고 이 파일만 쓴다.
 * - 앱 서버는 127.0.0.1:3006에만 바인딩한다(통합 3000, 데모 3001, 인증 3002, MEM 3003, FOOD 3004와 분리).
 ...
 *
 * 준비: `node tests/wishes/fixtures/cli.mjs setup`
 * 실행: `node node_modules/@playwright/test/cli.js test --config tests/wishes/playwright.config.ts`
 */

const REPO_ROOT = resolve(__dirname, '..', '..');
// 포트 배정(총괄): 통합 3000, 데모 3001, 인증 3002, MEM 3003, FOOD 3004, WISH 3006.
const PORT = Number(process.env.WISH_E2E_PORT ?? 3006);
...
if (supabaseUrl === '' || supabaseKey === '') {
  throw new Error(
    'WISH-001 E2E에는 로컬 Supabase 주소와 공개 키가 필요합니다. ' +
...
if (!['127.0.0.1', 'localhost', '[::1]', '::1'].includes(new URL(supabaseUrl).hostname)) {
  throw new Error('WISH-001 E2E는 로컬(loopback) Supabase에서만 실행합니다.');
}
...
export default defineConfig({
  ...
  // 초기화되는 불안정한 실패가 생긴다.
  outputDir: resolve(REPO_ROOT, '.agent-runtime', 'wish-e2e', 'test-results'),
