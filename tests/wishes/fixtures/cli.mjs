#!/usr/bin/env node
/**
 * WISH-001 E2E 픽스처 준비·정리 — 로컬 Supabase 전용.
 *
 *   node tests/wishes/fixtures/cli.mjs setup
 *   node tests/wishes/fixtures/cli.mjs teardown
 *   node tests/wishes/fixtures/cli.mjs status
 *
 * 네임스페이스: 합성 계정 `wish-e2e-{a,b,c}@test.invalid`만 만들고 지운다.
 *   - AUTH(`auth-e2e-*`)·MEM·FOOD(`food001-*`) 픽스처 계정은 읽지도 지우지도 않는다.
 ...
 *   - 비밀번호는 실행마다 새로 만들고 `.agent-runtime/wish-e2e/accounts.json`(Git 제외, 0600)에만 둔다.
 *   - 키·비밀번호·초대 토큰은 어떤 출력에도 넣지 않는다.
 *   - 컨테이너를 만들거나 지우거나 재시작하지 않는다. 이미 떠 있는 DB 컨테이너에서 psql만 쓴다.
 *
 * setup이 만드는 상태
 *   A(wish-e2e-a): 공간 생성 → B 초대 → B(wish-e2e-b) 수락 → 두 사람 공간
 *   C(wish-e2e-c): 별도 공간(외부 계정 역할)
 *   setup은 먼저 wish-e2e 계정의 공간·요청 기록을 지워 매 실행을 같은 출발점에서 시작한다.
 */
...
export const WISH_PREFIX = 'wish-e2e';
export const WISH_ACCOUNT_KEYS = /** @type {const} */ (['a', 'b', 'c']);

const HERE = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = resolve(HERE, '..', '..', '..');
const ACCOUNTS_PATH = join(REPO_ROOT, '.agent-runtime', 'wish-e2e', 'accounts.json');

function wishEmail(key) {
  const email = syntheticEmail(WISH_PREFIX, key);
  // 방어: 접두사가 바뀌어 다른 스위트 계정을 건드리는 일이 없게 한다.
  if (!email.startsWith(`${WISH_PREFIX}-`) || !isSyntheticEmail(email)) {
    fail('WISH-001 합성 계정만 다룹니다.');
  }
  return email;
}
...
function requireSql(config) {
  if (!canRunSql(config)) {
    fail(
      'AUTH_TEST_DB_CONTAINER가 필요합니다. 공간 생성 허용 목록 등록과 wish-e2e 데이터 초기화를 로컬 DB 컨테이너의 psql로 합니다.',
    );
  }
}
...
/**
 * 허용 목록에 wish-e2e 계정이 남아 있지 않은지 DB에서 다시 확인한다.
  ...
  return runSql(
    ...
    `do $$ begin
       if exists (select 1 from app_private.bootstrap_creators where email in (${list})) then
         raise exception 'wish-e2e 계정이 공간 생성 허용 목록에 남아 있다';
       end if;
     end $$;`,
  );
}

/** wish-e2e 계정의 공간·요청 기록이 남아 있지 않은지 확인한다. */
  ...
  return runSql(
    ...
    `do $$ begin
       if exists (select 1 from public.space_members m join auth.users u on u.id = m.user_id
         ...
         raise exception 'wish-e2e 공간·요청 기록이 남아 있다';
       end if;
     end $$;`,
  );
}
...
function removeBootstrap(config) {
  const problems = [];
  const creators = [wishEmail('a'), wishEmail('c')];
...
async function setup(config) {
  ...
  const emails = WISH_ACCOUNT_KEYS.map(wishEmail);

  // 매 실행을 같은 출발점에서: wish-e2e 계정의 공간(위시 포함)과 요청 기록만 지운다.
  // 공간 삭제가 wish_items를 cascade로 지우므로 별도 삭제문이 필요 없다.
  checkSql(purgeSyntheticData(config, emails), 'wish-e2e 기존 데이터 정리');
  checkSql(verifyDataPurged(config, emails), 'wish-e2e 기존 데이터 정리 확인');

  const accounts = {};
  for (const key of WISH_ACCOUNT_KEYS) {
    const email = wishEmail(key);
...
async function teardown(config) {
  ...
  else console.log('wish-e2e 공간·요청 기록 정리 확인');
