...
import { MEMORY_BUCKET, MEMORY_PHOTO_MAX_BYTES, isMemoryUploadMime } from '@/features/memories/live/constants';
...
/**
 * 추억 사진 전달(인증된 다운로드). DESIGN.md 8.3의 기본안.
 *
 * - 요청한 사용자의 세션으로 asset 행과 Storage 객체를 **각각 RLS/Storage 정책으로** 읽는다.
 *   service_role·서명 URL·공개 URL을 쓰지 않는다.
 * - 보이는 것: 업로더 본인의 자기 파일, 같은 공간의 ready + 실제 첨부 파일. `deleting`은 누구에게도 안 보인다.
 * - 비로그인·외부 계정·상대의 미첨부 파일·없는 ID·형식이 틀린 ID는 모두 같은 404다.
...
export async function GET(
  ...
  if (asset.state === 'deleting' || asset.purpose !== 'memory' || !isMemoryUploadMime(asset.mime_type)) {
    return notFound();
  }
  ...
  if (downloadError || !blob || blob.size === 0 || blob.size > MEMORY_PHOTO_MAX_BYTES) {
    return notFound();
  }
