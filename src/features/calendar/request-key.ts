 ...
 *   - 확정 응답(성공·입력 오류·충돌·권한 없음·없음) → 키를 버린다. 다음 제출은 새 작업이므로 새 키를 쓴다.
 *   - 입력을 바꾸면 다른 작업이다. 같은 키로 다른 입력을 보내면 DB가 CONFLICT로 거부하므로 새 키를 쓴다.
 *
 * (위시 기능에 같은 규칙의 파일이 있다. 두 기능이 서로의 파일을 고치지 않도록 각자 둔다.
 *  통합 뒤 총괄이 공용 위치로 옮길 수 있다.)
 */
 ...
 * 응답 뒤 관문.
 *
 * - `lockOnSuccess`(성공하면 다른 화면으로 이동하는 작업: 일정 등록·수정, 일정 삭제):
...
export function finishMutation(
  ...
  return {
    key: settleRequestKey(gate.key, outcome.ok || outcome.definitive),
    running: false,
    locked: false,
  };
}
