...
import { loadMoreWishesAction } from '../actions';

import { useHydrated } from './use-hydrated';
import { WishCard } from './WishCard';

import type { WishCursor, WishFilters } from '../filters';
 ...
 * - 같은 위시가 두 번 붙지 않게 ID로 거른다(새로 추가된 항목과 겹칠 때 대비).
 */
export function WishList({
  ...
  return (
    <div className="space-y-4">
      <ul className="grid gap-3 sm:grid-cols-2" aria-label="위시 목록" data-testid="wish-list">
        {items.map((item) => (
          <li key={item.id}>
            <WishCard wish={item} />
          </li>
