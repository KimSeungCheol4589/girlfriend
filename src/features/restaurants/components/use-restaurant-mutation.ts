...
import { isDefinitiveCode, type RestaurantFailure } from '../errors';
import { beginMutation, finishMutation, OPEN_GATE, signatureOf, type MutationGate } from '../request-key';

import type { RestaurantActionResult } from '../actions';

/** Server Action 호출 자체가 실패한 경우(응답 없음). 저장 여부를 단정하지 않는다. */
export const NETWORK_FAILURE: RestaurantFailure = {
  ok: false,
  code: 'UNKNOWN',
  message:
 ...
 *   클릭이 새 키로 두 번째 생성을 만들던 결함(E2E v2)을 막는다. `locked`로 화면을 비활성 상태로 유지한다.
 * - DB의 requestId가 두 번째 방어선이다.
 */
export function useRestaurantMutation<TPayload extends Record<string, unknown>, TData>(
  operation: string,
  action: (input: TPayload & { requestId: string }) => Promise<RestaurantActionResult<TData>>,
