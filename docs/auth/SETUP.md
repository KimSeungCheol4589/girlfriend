...
관리자 키(`service_role`)는 **앱의 일반 요청 경로에 쓰지 않는다.** 일반 조회·저장은 사용자 세션과 RLS만 쓴다.
관리자 키가 필요한 작업은 운영자의 수동 절차(아래), 로컬 테스트 픽스처, 그리고 2.2절의
**추억 사진 확정·정리 예외(MEM-001)** 뿐이다.
...
### 2.2 예외: 추억 사진 확정·정리용 서버 비밀 (MEM-001)

DB 계약상 `finalize_upload`는 로그인 사용자가 실행할 수 없고(service 전용), Storage 삭제 정책도 사용자에게 없다.
그래서 추억 사진에 한해 앱 서버가 별도 이름의 서버 비밀 `MEM_SUPABASE_SERVICE_ROLE_KEY`를 읽는다.

- 읽는 곳은 `src/features/memories/server/service-client.ts` 하나이며 `server-only`로 브라우저 번들 포함을 막는다.
  요청 세션 클라이언트(`src/lib/supabase/server.ts`)와 **별개의 객체**다. 세션 쿠키를 싣지 않는다.
- 쓰는 곳: (1) 사진 확정 — Auth `getUser`로 사용자를 확인하고 **사용자 RLS로** pending asset의
  업로더·공간·목적·경로를 확인한 **뒤에만** 객체를 읽기 전용으로 내려받아 검증하고 `finalize_upload`를 부른다.
  객체를 덮어쓰거나 다시 인코딩하지 않는다. (2) 정리 — DB 응답이 알려 준 `deleting` 경로만 재확인 후 삭제.
- 일반 조회·저장·삭제·고정은 계속 사용자 세션만 쓴다.
- 값이 없으면 사진 올리기만 꺼지고 화면이 그 사실을 밝힌다. 가짜 성공을 보여 주지 않는다.
- 배포 비밀 저장소에만 둔다. 로그·오류 보고·`NEXT_PUBLIC_` 이름에 넣지 않는다.
- 인증 E2E 설정(`tests/auth/web-server-env.ts`)은 이름에 `SERVICE_ROLE`이 든 이 키도 빈 값으로 덮어쓴다.
  추억 E2E(`playwright.memories.config.ts`)만 이 이름 하나를 앱 서버에 넘긴다.
