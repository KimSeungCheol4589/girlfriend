...
1. **실행 검증이 일부만 끝났다.** 최초 11개 마이그레이션은 적용에 성공했고
   `20_privileges`·`30_rls_visibility`·`40_space_and_invites`가 통과했다.
   `10_schema_constraints`·`50_mutations`는 정규식 결함으로 실패했고, 동시성은 지연 트리거 결함으로 실패했다.
   이번 수정 마이그레이션 3개와 새 테스트(60, 70)는 **아직 실행되지 않았다.**
