...
### 총괄이 실행한 최종 확인 (head `9467970`)

이 세션이 아니라 **총괄이 로컬에서** 실행한 결과다.

| 항목 | 결과 |
| --- | --- |
| QA 375px 모바일 검사 | **1 pass** |
| 단위 테스트 | 828 pass |
...
QA 스위트를 직접 실행한 적이 없다. 이후 이 파일들을 고칠 때는 총괄 재실행이 다시 필요하다
(이번 정리에서 판정 기준·진단 분류·회귀 테스트가 바뀌었으므로, 다음 실행 때 수치와 함께 확인하면 좋다).
